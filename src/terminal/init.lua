...
-- Use scheduler-provided mutex (e.g., copas.mutex or ngx.semaphore)
local mutex = require("copas.mutex") -- or ngx.semaphore

-- Generalized query function
function terminal.query(cmd, response_pattern, timeout)
  mutex:lock()
  local response = ""
  ...
  -- Read the response using system.readansi()
  local start_time = os.time()
  while true do
      local data = system.readansi()
      if data then
          response = response .. data
          if response:match(response_pattern) then
              break
          end
      end
...
function terminal.read_custom()
  ...
  return system.readansi() -- Use system.readansi() instead of io.read()
end
